...
ENV_FILE="/etc/mtproxy/mtproxy.env"  # Секреты для systemd EnvironmentFile (0600)
PROXY_TAG=""
...
chown root:root "$CONFIG_DIR/secret" "$CONFIG_DIR/domain"

# Секрет вынесен в EnvironmentFile, чтобы не хранить его напрямую в unit-файле.
cat > "$ENV_FILE" <<EOF
...
# Минимизируем writable-поверхность: код и бинари принадлежат root.
chown -R root:root "$INSTALL_DIR"
chmod -R go-w "$INSTALL_DIR"
# MTProxy после setuid должен читать конфиги Telegram.
chown mtproxy:mtproxy "$INSTALL_DIR/proxy-secret" "$INSTALL_DIR/proxy-multi.conf"
chmod 640 "$INSTALL_DIR/proxy-secret" "$INSTALL_DIR/proxy-multi.conf"
